...
import { ShoppingCart, Trash2, Plus, Minus, X, PackageOpen, Package, Receipt, Gift, CheckSquare, Edit2 } from 'lucide-react';
            ...
            {items.length > 0 && (
              <>
                <button
                  ...
                  className="text-[10px] font-bold px-2 py-1.5 rounded-xl border border-amber-200 dark:border-amber-900/50 text-amber-700 dark:text-amber-500 bg-amber-50 dark:bg-amber-500/10 hover:bg-amber-100 dark:hover:bg-amber-500/20 transition-all focus:outline-none focus:ring-2 focus:ring-amber-500/50 active:scale-[0.95]"
                          ...
                          <input 
                            ...
                            onKeyDown={(e) => {
                              if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
                                e.preventDefault();
                                const currentIdx = parseInt(e.target.getAttribute('data-cart-index'), 10);
                                const nextIdx = e.key === 'ArrowUp' ? currentIdx - 1 : currentIdx + 1;
                                const nextInput = document.querySelector(`.cart-qty-input[data-cart-index="${nextIdx}"]`);
                                if (nextInput) {
                                  nextInput.focus();
                                }
                              }
