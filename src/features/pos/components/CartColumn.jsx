...
export default function CartColumn({ 
  ...
  const handleToggleAllWholesale = () => {
    // Wholesale toggle removed
  };

  return (
    <div className="flex flex-col h-full bg-transparent relative transition-colors duration-500">
      <div className="px-6 py-4 border-b border-white/10 flex flex-col gap-3 glass-panel z-10 sticky top-0 flex-shrink-0">
        ...
        {cartTabs && cartTabs.length > 0 && (
          <div className="relative">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-1 hide-scrollbar">
                ...
                return (
                  <div 
                    ...
                    className={`flex items-center rounded-xl transition-all border flex-shrink-0 cursor-pointer ${
                      isActive 
                        ? 'glass-card border-white/30 text-sky-700 dark:text-sky-300 font-bold shadow-md' 
                        : 'glass-panel border-white/10 text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'
                    }`}
                  ...
                  >
                    ...
                    <button 
                      ...
                      className={`p-1.5 rounded-r-xl transition-all hover:text-rose-500 active:scale-90 ${isActive ? 'glass-button' : 'hover:bg-slate-200 dark:hover:bg-slate-700'}`}
                      title="Đóng đơn này"
              ...
              <button 
                onClick={onAddTab}
                className="flex items-center justify-center min-w-[28px] h-[28px] rounded-xl border border-dashed border-slate-300/50 dark:border-slate-600/50 text-slate-400 hover:text-sky-500 hover:border-sky-300 glass-button transition-colors flex-shrink-0"
          ...
          <div className="flex flex-col gap-3">
            <AnimatePresence>
              {items.map((item, index) => {
                ...
                let bgClass = 'glass-card hover:shadow-md';
                if (isWholesale) bgClass = 'glass-card border-amber-500/20';
                if (isMid) bgClass = 'glass-card border-sky-500/20';
                ...
                let qtyControlsBg = 'glass-panel';
                if (isWholesale) qtyControlsBg = 'glass-panel border-amber-500/20';
                if (isMid) qtyControlsBg = 'glass-panel border-sky-500/20';
                ...
                return (
                  <motion.div 
                    ...
                    className={`flex flex-col gap-3 p-4 rounded-2xl border transition-all group ${bgClass}`}
                  >
                    {/* Row 1: Name & Delete */}
                    <div className="flex justify-between items-start gap-3">
                      <div className="flex-1">
                        <h4 className={`font-black text-lg leading-tight ${textClass}`}>
                          {isWholesale && <Package size={18} className="text-amber-500 inline-block mr-1.5 align-text-bottom" />}
                          {isMid && <PackageOpen size={18} className="text-sky-600 dark:text-sky-400 inline-block mr-1.5 align-text-bottom" />}
                          {item.name}
                        </h4>
                        <div className="flex items-center gap-3 mt-1">
                          <span className="text-slate-500 font-mono tracking-wide text-sm">
                            {item.barcode}
                          </span>
                          {isWarningStock && (
                      ...
                      <motion.button 
                        ...
                        className="text-rose-500 hover:text-white glass-button hover:bg-rose-500/80 p-2 rounded-xl transition-colors focus:outline-none flex-shrink-0 flex items-center gap-1.5"
                        title="Xóa sản phẩm này"
                        aria-label="Xóa sản phẩm này"
                      >
                        <Trash2 size={18} strokeWidth={2.5} />
                        <span className="font-bold text-xs hidden sm:inline uppercase">Xóa</span>
                      </motion.button>
                    </div>
                      ...
                      <div className="text-right flex items-center gap-2">
                        <div className="text-right flex flex-col justify-end">
                          {editingCartId === item.cartId ? (
                            <div 
                              className="flex items-center gap-2 glass-input border-rose-200/50 p-2 rounded-xl w-fit"
                            ...
                            <div className="flex items-center gap-1.5 justify-end">
                              ...
                              {item.customDiscount !== undefined && item.customDiscount > 0 && (
                                <span className="text-[10px] glass-panel text-rose-700 dark:text-rose-300 px-1.5 py-0.5 rounded font-extrabold tracking-wider">
                          ...
                          {getEffectiveTaxRate && getEffectiveTaxRate(item) > 0 && (
                            <span className="text-xs font-bold text-rose-600 glass-panel px-2.5 py-1 rounded-lg mt-1 self-end">
                    ...
                    <div className="flex items-center justify-between mt-1 pt-3 border-t border-white/10">
                      <div className={`flex items-center rounded-xl overflow-hidden h-11 ${qtyControlsBg} shadow-sm`}>
                        <motion.button 
                          whileTap={{ scale: 0.9 }}
                          onClick={() => onUpdateQty(item.cartId, -1)}
                          className={`w-10 sm:w-12 h-full flex items-center justify-center transition-colors focus:outline-none border-r ${qtyBtnClass} border-white/10`}
                        ...
                        >
                          ...
                          <input 
                            ...
                            className={`cart-qty-input w-12 sm:w-16 h-full text-center font-black text-xl sm:text-2xl glass-input focus:outline-none transition-all ${textClass}`}
                            min="0.01"
                          />
                        <motion.button 
                          whileTap={{ scale: 0.9 }}
                          onClick={() => onUpdateQty(item.cartId, 1)}
                          className={`w-10 sm:w-12 h-full flex items-center justify-center transition-colors focus:outline-none border-l ${qtyBtnClass} border-white/10`}
                      ...
                      <div className={`font-black text-2xl sm:text-3xl ${isCredit ? 'text-rose-600 dark:text-rose-400' : 'text-sky-700 dark:text-sky-400'}`}>
                    ...
                    {(() => {
                      ...
                      if (promo) {
                        ...
                        if (hintText) {
                          return (
                            <div className={`mt-3 text-xs sm:text-sm font-bold px-3 py-2.5 rounded-xl border flex items-center justify-center gap-2 text-center shadow-sm glass-panel ${
                              isSuccess 
                                ? 'text-emerald-600 border-emerald-500/30'
                                : 'text-rose-600 border-rose-500/30 animate-pulse'
                            }`}>
                    ...
                    {activeDiscountTier && (
                      <div className="mt-3 text-xs sm:text-sm font-bold px-3 py-2.5 rounded-xl glass-panel text-emerald-600 border border-emerald-500/30 flex items-center justify-center gap-2 text-center shadow-sm">
      ...
      <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-t border-slate-200/50 dark:border-slate-800/50 p-4 sm:p-5 flex-shrink-0 flex flex-col gap-3 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] dark:shadow-[0_-10px_30px_rgba(0,0,0,0.2)]">
        ...
        <motion.button
          ...
          className={`w-full py-4 rounded-2xl flex items-center justify-center gap-2 text-base font-black uppercase tracking-wider transition-all duration-300 focus:outline-none focus-visible:ring-4 focus-visible:ring-sky-500/50 ${
            items.length === 0
              ? 'glass-button text-slate-400 dark:text-slate-600 cursor-not-allowed opacity-50' 
              : isCredit 
                ...
                : 'bg-gradient-to-r from-sky-500/90 to-blue-600/90 hover:from-sky-500 hover:to-blue-600 text-white shadow-lg shadow-sky-500/30 backdrop-blur-md border border-white/20'
          }`}
        >
